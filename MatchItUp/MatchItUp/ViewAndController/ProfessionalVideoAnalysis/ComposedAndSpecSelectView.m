...
@interface ComposedAndSpecSelectView ()<CanvasViewDelegate, scoreCanvasDelegate,UICollectionViewDelegate, UICollectionViewDataSource>
...
@property (nonatomic, strong) NSMutableArray<SpecificationAsset*> *specModels;
@property (nonatomic, strong) NSMutableArray<SpecificationAsset*> *didModels;
@property (nonatomic, strong) NSMutableDictionary *specJsonData;
@property (nonatomic, strong) NSMutableDictionary *saveSpecUuid;
@end

@implementation ComposedAndSpecSelectView {
    ...
    NSMutableDictionary *zhiyeModelTools;
...
//    MBProgressHUD *autoDrawLineProgressHUD;
    ...
    UIButton *uplodButton;
    UICollectionView *canSelectcollectionView;
    UICollectionView *didSelectcollectionView;
}
...
- (void)mas_makeConstraintsFor:(UIButton *)pencilBtn and:(UIButton *)eraseBtn and:(UIButton *)deleteAllBtn and:(UIButton *)lineBtn and:(UIButton *)redBtn and:(UIButton *)rectBtn and:(UIButton *)yellowBtn and:(UIButton *)angleBtn and:(UIButton *)greenBtn and:(UIButton *)circleBtn and:(UIButton *)blueBtn and:(UIButton *)curveBtn and:(UIButton *)blackBtn {
    ...
    [pencilBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(bottomView).multipliedBy(0.15);
    }];
...
- (instancetype)initWithFrame:(CGRect)frame andisFront:(BOOL)isFrontParam andVideoURL:(NSURL *)videoURL andVideo:(nonnull Video *)video andFrameIndexArray:(NSMutableArray*)frameIndexArray andSpecModels:(NSMutableArray*)models
{
    ...
    if (self) {
        ...
        _video = video;
        _specModels = models;
        _didModels = [[NSMutableArray alloc]init];
        self.backgroundColor = [UIColor blackColor];
        self.canvasToolArray = [[NSMutableArray alloc] init];
        _specJsonData = [[NSMutableDictionary alloc]init];
        _saveSpecUuid = [[NSMutableDictionary alloc]init];
        
        bottomView = [[UIImageView alloc] init];
        [bottomView setImage:[UIImage imageNamed:@"topView"]];
//        bottomView.backgroundColor = [UIColor clearColor];
//        self.backgroundColor = [UIColor redColor];
        bottomView.userInteractionEnabled = YES;
        bottomViewH = 50;
        bottomView.frame = CGRectMake(0, 0, self.frame.size.width, bottomViewH);
        ...
        for (int i = 0; i < 13; i++) {
            UIImageView *tmpView;
            tmpView = [[UIImageView alloc] initWithImage:[UIImage imageWithData:frameIndexArray[i]]];
            if (i == 4 || i == 8) {
                UIImageView *tmpView1;
                tmpView1 = [[UIImageView alloc] initWithImage:[UIImage imageWithData:frameIndexArray[i]]];
        ...
        scrollView.frame = CGRectMake(0, bottomViewH, self.frame.size.width, ((self.frame.size.width - 10) / 4) * 3);
        ...
        selectView.backgroundColor = [UIColor colorWithRed:53/255.f green:53/255.f blue:53/255.f alpha:1];
        selectView.frame = CGRectMake(0, ((self.frame.size.width - 10) / 4) * 3+bottomViewH, self.frame.size.width, self.frame.size.height-bottomViewH-((self.frame.size.width - 10) / 4) * 3);
        [self addSubview:selectView];
        
        uplodButton = [[UIButton alloc]init];
        [uplodButton addTarget:self action:@selector(autoDrawLines) forControlEvents:UIControlEventTouchUpInside];
        [uplodButton setFrame:CGRectMake(0, 0, 55, 55)];
        [uplodButton setCenter:CGPointMake(self.frame.size.width/12*11, 340)];
        [uplodButton setImage:[UIImage imageNamed:@"编辑"] forState:UIControlStateNormal];
        [selectView addSubview:uplodButton];
        
        resetButton = [[UIButton alloc]init];
        [resetButton addTarget:self action:@selector(resetSpecModel) forControlEvents:UIControlEventTouchUpInside];
        [resetButton setFrame:CGRectMake(0, 0, 55, 55)];
        [resetButton setCenter:CGPointMake(self.frame.size.width/12, 340)];
        [resetButton setImage:[UIImage imageNamed:@"cancel"] forState:UIControlStateNormal];
        [selectView addSubview:resetButton];
        ...
        if(keys){
            NSMutableArray<SpecificationAsset*> *models = [[SpecificationDataModel alloc] initWithSpecificatins:[[CoreDataManager sharedManager] getSpecificationOfUsingWith:keys]].assets;
            for (SpecificationAsset *asset in models) {
                for (int i = 0; i < _specModels.count; i++) {
                    SpecificationAsset *asset2  = _specModels[i];
                    if(asset.uuid == asset2.uuid){
                        [_didModels addObject:asset2];
                    }
                }
            }
        }
        ...
        [backBtn setCenter:CGPointMake(bottomView.frame.size.width / 7 * 6.5, 25)];
        ...
        UILabel *topLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 35, self.frame.size.width, 30)];
        [topLabel setFont: [UIFont boldSystemFontOfSize:18]];
        [topLabel setTextAlignment:NSTextAlignmentCenter];
        topLabel.text = @"Technique  Indicators";
        topLabel.textColor = [UIColor whiteColor];
        [selectView addSubview:topLabel];
        
        UICollectionViewFlowLayout *layout1 = [[UICollectionViewFlowLayout alloc] init];
        layout1.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        layout1.minimumInteritemSpacing = 0;
        layout1.minimumLineSpacing = 0;
        layout1.itemSize = CGSizeMake(75 ,75);


        canSelectcollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(self.frame.size.width/6,95,self.frame.size.width/3*2,75) collectionViewLayout:layout1];
        canSelectcollectionView.contentSize = CGSizeMake(self.frame.size.width/3*2,75);
        canSelectcollectionView.backgroundColor = [UIColor clearColor];
        canSelectcollectionView.dataSource = self;
        canSelectcollectionView.delegate = self;
        canSelectcollectionView.pagingEnabled = YES;
        canSelectcollectionView.scrollEnabled = YES;
        [canSelectcollectionView registerClass:[FrameCollectionViewCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
        [selectView addSubview:canSelectcollectionView];
        if(_specModels.count<8){
            CGFloat collectionViewWidth = canSelectcollectionView.bounds.size.width;
            CGFloat cellWidth = 75*_specModels.count;
            CGFloat inset = (collectionViewWidth - cellWidth) / 2.0;
            canSelectcollectionView.contentInset = UIEdgeInsetsMake(0, inset, 0, inset);
        }
        ...
        didSelectcollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0,0,self.frame.size.width/3*2,75) collectionViewLayout:layout2];
        [didSelectcollectionView setCenter:CGPointMake(self.frame.size.width/2, 340)];
        
        didSelectcollectionView.backgroundColor = [UIColor blackColor];
        didSelectcollectionView.dataSource = self;
        didSelectcollectionView.delegate = self;
        [didSelectcollectionView registerClass:[FrameCollectionViewCell class] forCellWithReuseIdentifier:@"didSelect"];
        [selectView addSubview:didSelectcollectionView];
        if(_didModels.count<8){
            CGFloat collectionViewWidth = canSelectcollectionView.bounds.size.width;
            CGFloat cellWidth = 75*_didModels.count;
            CGFloat inset = (collectionViewWidth - cellWidth) / 2.0;
            didSelectcollectionView.contentInset = UIEdgeInsetsMake(0, inset, 0, inset);
        }
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hasChangeEidte) name:@"ChangeEditeStateYes" object:nil];
        ...
        [self displayAutoDrawLinesResult];
    }
    return self;
}
...
- (void)hasChangeEidte {
    [[CoreDataManager sharedManager] changeEditeForVideo:_video];
}
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale {
    ...
    [zhiyeModelTools enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [obj enumerateObjectsUsingBlock:^(NSArray *toolsArray, NSUInteger idx, BOOL * _Nonnull stop) {
            for (SpecificationTool *objTool in toolsArray) {
//                [objTool updateWithContentSize:scrollView.contentSize];
                
                if([objTool.corresFrame intValue] <= 4){
                    [objTool updateWithContentSize:scrollView.contentSize andIndex:[objTool.corresFrame intValue]];
                    if([objTool.corresFrame intValue] == 4){
                        [objTool updateWithContentSize:scrollView.contentSize andIndex:[objTool.corresFrame intValue]+1];
                    }
                }else if ([objTool.corresFrame intValue]>4 && [objTool.corresFrame intValue] <=8){
                    [objTool updateWithContentSize:scrollView.contentSize andIndex:[objTool.corresFrame intValue]+1];
                    if([objTool.corresFrame intValue] == 4){
                        [objTool updateWithContentSize:scrollView.contentSize andIndex:[objTool.corresFrame intValue]+2];
                    }
                }else{
                    [objTool updateWithContentSize:scrollView.contentSize andIndex:[objTool.corresFrame intValue]+2];
                }
            ...
            switch (status) {
                ...
                case -1:
                    if (self->lastState != -1) {
                        ...
                        [def setObject:@4 forKey:[NSString stringWithFormat:@"videoModelState%@",self->videoId]];
                        [def synchronize];
                        [self.delegate videoModeChange:@4];
                    }
...
- (void)uplod13Pics {
    ...
    [manager POST:@"http://219.238.233.6:37577/progolf/upload" parameters:@{@"type" : @1} headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            [self->_specJsonData enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                [formData appendPartWithFileData:obj name:@"file" fileName:[key stringByAppendingString:@".json"] mimeType:@"json"];
            }];
...
//                    self.autoChooseFrameBtn.enabled = false;
                    ...
                    NSString *didSelectSpecSavePath = [filePath stringByAppendingPathComponent:@"didSelectSpec"];
                    if([self->_saveSpecUuid writeToFile:didSelectSpecSavePath atomically:YES]){
                        NSLog(@"选择指标保存成功");
                    }else{
                        NSLog(@"选择指标保存失败");
                    }
...
- (void)displayAutoDrawLinesResult {
    ...
    NSDictionary *zhiyeModelData = [NSKeyedUnarchiver unarchiveObjectWithFile:toolsSavePath];
    if (zhiyeModelData) {
        // 移除之前的
        [zhiyeModelTools enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            [obj enumerateObjectsUsingBlock:^(NSArray *toolsArray, NSUInteger idx, BOOL * _Nonnull stop) {
                if(toolsArray.count!=0){
                    for (SpecificationTool *objTool in toolsArray) {
                        [objTool.toolLayer removeFromSuperlayer];
                    }
                }
            }];
        }];
        ...
        zhiyeModelTools = [[NSMutableDictionary alloc]initWithDictionary:zhiyeModelData];
//        [self showScoreTools];
        [self showSpecModelTools];
    }
    NSString *videoSavePath = [filePath stringByAppendingPathComponent:@"out_up.mp4"];
    if([[NSFileManager defaultManager] fileExistsAtPath:videoSavePath]){
        [self playVideoPIP];
...
- (void)showSpecModelTools{
    [zhiyeModelTools enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [obj enumerateObjectsUsingBlock:^(NSArray *toolsArray, NSUInteger idx, BOOL * _Nonnull stop) {
            for (SpecificationTool *objTool in toolsArray) {
                if([objTool.corresFrame intValue] <= 4){
                    [_scoreCanvas drawCanvasWithTool:objTool andIndex:[objTool.corresFrame intValue]];
                    if([objTool.corresFrame intValue] == 4){
                        [_scoreCanvas drawCanvasWithTool:objTool andIndex:[objTool.corresFrame intValue]+1];
                    }
                }else if ([objTool.corresFrame intValue]>4 && [objTool.corresFrame intValue] <=8){
                    [_scoreCanvas drawCanvasWithTool:objTool andIndex:[objTool.corresFrame intValue]+1];
                    if([objTool.corresFrame intValue] == 4){
                        [_scoreCanvas drawCanvasWithTool:objTool andIndex:[objTool.corresFrame intValue]+2];
                    }
                }else{
                    [_scoreCanvas drawCanvasWithTool:objTool andIndex:[objTool.corresFrame intValue]+2];
                }
...
- (void)resetSpecModel{
    [_didModels removeAllObjects];
    [_specJsonData removeAllObjects];
    [_saveSpecUuid removeAllObjects];
    [didSelectcollectionView reloadData];
    [canSelectcollectionView reloadData];
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    if([collectionView isEqual:canSelectcollectionView]){
        return _specModels.count;
    }
    return _didModels.count;
}
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if(collectionView==canSelectcollectionView){
        ...
        asset = _specModels[indexPath.row];
        [cell setFrameImg:asset.cover withRate:0];
        if([_didModels containsObject:asset]){
            [cell selectCell];
            cell.userInteractionEnabled=NO;
        }
        return cell;
    }else{
        FrameCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"didSelect" forIndexPath:indexPath];
        SpecificationAsset *asset = nil;
        asset = _didModels[indexPath.row];
        [cell setFrameImg:asset.cover withRate:0];
        return cell;
    }
    return nil;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if(collectionView==canSelectcollectionView){
        ...
        SpecificationAsset *asset = nil;
        asset = _specModels[indexPath.row];
        [_didModels addObject:asset];
        [didSelectcollectionView reloadData];
        [canSelectcollectionView reloadItemsAtIndexPaths:@[indexPath]];
        if(_didModels.count<8){
            CGFloat collectionViewWidth = canSelectcollectionView.bounds.size.width;
            CGFloat cellWidth = 75*_didModels.count;
            CGFloat inset = (collectionViewWidth - cellWidth) / 2.0;
        ...
        NSString *jsonPath = [asset.modelFile stringByAppendingPathComponent:@"jsonData.json"];
        NSData *jsonData = [NSData dataWithContentsOfFile:jsonPath];
        [_specJsonData setObject:jsonData forKey:asset.uuid];
        [_saveSpecUuid setObject:asset.model.modelFile forKey:asset.uuid];
    }
}
