...
void Bvh::mdlProcess(std::string name, std::string value, std::string club_parent) {
...
//        clubRoot->parent = NULL;
                // To fix the club and hand position bug, change the parent of "club" from "global" to "h_left_hand" h_left_low_fing_1 lcy
                clubRoot->parent = getBvhPart(bodyRoot, club_parent);
                //clubRoot->parent = getBvhPart(bodyRoot, "h_left_low_fing_1");
                current->parent->child.push_back(current);
            } else if(current->name != "left_foot_plate" && current->name != "right_foot_plate"){
...
void Bvh::mdlParse(std::string mdlFile, string club_parent) {
...
//                    current->m.tz = ::atof(vec[2].c_str());
                    //将mdl
                    current->m.tx = current->matrix.tx;
                    current->m.ty = current->matrix.ty;
                    current->m.tz = current->matrix.tz;
                }
...
//            matrix.tx = value;
        ...
        //      printf("%d, %f \n",order,value);
//        if(part->name=="length"){
//            cout<<v.x<<" "<<dataId<<endl;
//        }
    }
//    if(part->name=="length"){
//        cout<<v.x<<" "<<part->orders.size()<<endl;
//    }
...
void Bvh::setPartLen(string name,float length,float prop=0)
{
    ...
    if(prop){
        if(current->child.size()==1){
            ...
            tx=current->child[0]->m.tx;
            ty=current->child[0]->m.ty;
            tz=current->child[0]->m.tz;
            Vector3 t0(tx,ty,tz);
            tmag=vectorMag(t0);
...
//            cout<<"maxl:"<<current->name<<current->skinlen<<endl;
        }
        for(unsigned int j=0;j<current->child.size();j++)
...
void Bvh::computeHeight()
{
    ...
    Matrix4x3 mathead=bodyName["h_head"]->gm; //bodyName是bvhpart构成的map
    Matrix4x3 matrfoot=bodyName["h_right_foot"]->gm;
    Matrix4x3 matlfoot=bodyName["h_left_foot"]->gm;
...
void Bvh::computeVexNormal(BvhPart* part)
{
    ...
    for(int i=0;i<part->faces.size();i++){
        ...
        normal.normalize();
        for(int j=0;j<part->faces[i].size();j++){
            int id =part->faces[i][j];
            part->vexnormals[id]+=normal;
        }
...
void Bvh::setDetailBodySize(float height,float weight,float waistgirth,std::map<std::string,float> lengthmap,std::map<std::string,float> girthmap)
{
    ...
    float waist_torso_len=bodyName["h_waist"]->skinlen+bodyName["h_torso_7"]->skinlen+bodyName["h_torso_6"]->skinlen+bodyName["h_torso_5"]->skinlen+bodyName["h_torso_4"]->skinlen+bodyName["h_torso_3"]->skinlen+bodyName["h_torso_2"]->skinlen;
    ...
    float prop=torso_len/new_torso_len;
    cout<<"prop:"<<prop<<endl;
//    float llen=0,new_len=0;
    
//    prop=(height-new_len)/(height-llen);
    for(auto si:lengthmap){
        lengthmap[si.first]=bodyName[si.first]->skinlen;
        setPartLen(si.first,si.second);
        cout<<"input:"<<si.first<<" "<<si.second<<endl;
//        llen+=bodyName[si.first]->skinlen;
//        new_len+=si.second;
    }
    float tp=1;
    float v;
...
//    setPartLen("h_left_wrist",len);
    ...
    setPartLen("h_torso_7",0,prop);
//    setPartLen("h_torso_2",bodyName["h_torso_2"]->skinlen);
//    setPartLen("h_torso_3",bodyName["h_torso_3"]->skinlen);
//    setPartLen("h_torso_4",bodyName["h_torso_4"]->skinlen);
//    setPartLen("h_torso_5",bodyName["h_torso_5"]->skinlen);
//    setPartLen("h_torso_6",bodyName["h_torso_6"]->skinlen);
//    setPartLen("h_torso_7",bodyName["h_torso_7"]->skinlen);
